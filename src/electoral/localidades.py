...
- `NIVEL_OFICIAL` ("oficial_confirmada"): Resolución 1990/2007. La mayoría
  de estos circuitos tiene una única localidad dominante, pero no es
  requisito -- alcanza con que la localidad figure textualmente en la tabla
  oficial del circuito (ver decisión de interpretación #4 en
  `AUDITORIA_DISCREPANCIAS.md`, sobre 503 y 503A -> MELCHOR_ROMERO).
- `NIVEL_PERIODISTICO` ("periodistico_no_oficial"): relevamiento de El Día,
  no administrativo.
...
def mapa_localidad_por_circuito(
    ...
    niveles_cobertura: tuple[str, ...] = (NIVEL_OFICIAL,),
) -> dict[str, str]:
    """circuito_id -> localidad, aplicando la precedencia oficial > periodístico
    y excluyendo siempre los circuitos marcados `oficial_no_agrupable`.
    """
    mapa: dict[str, str] = {}
    for nivel in (NIVEL_PERIODISTICO, NIVEL_OFICIAL):  # oficial se procesa último: pisa
...
class ReporteCobertura:
    ...
    def __str__(self) -> str:
        ...
        return (
            ...
            f"({self.porcentaje_votos:.1f}%) -- niveles usados: {', '.join(self.niveles_cobertura)}. "
...
def agrupar_resultados_por_localidad(
    ...
    for circuito_id, valores in resultados_por_circuito.items():
        ...
        localidad = mapa.get(circuito_id, SIN_DETERMINAR)
    ...
    reporte = ReporteCobertura(
        ...
        niveles_cobertura=tuple(niveles_cobertura),
    )
