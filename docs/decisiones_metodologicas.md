...
| D20 | **Validación del oficialismo inicial en la ventana 2001-2003 y retiro de `sin_oficialismo`**: `especificacion_panel_temporal.md` §6.2 preveía una cuarta categoría de `continuidad_oficialismo`, `sin_oficialismo`, para "casos extremos (2001-2003, colapso del gobierno)" -- referencia a la crisis de diciembre de 2001 (renuncia de De la Rúa, sucesión de varios presidentes en dos semanas). Verificado que esa categoría **nunca se activa** en el pipeline real, por dos motivos distintos según el nivel. A nivel **nacional**, la ventana 2001-2003 directamente no existe en el panel: `calendario_electoral.csv`/`oficialismo_por_nivel.csv` no cubren nacional antes de 2011 (D15) -- `data/tfi_data/elecciones/2001_nacional.csv` a `2009_nacional.csv` existen pero están vacíos (solo encabezado), a diferencia de sus pares `municipal`/`provincial` (25 filas cada uno): nunca se consiguió el escrutinio por lista a nivel Partido de La Plata para el cargo nacional en 2001-2009 (`docs/adquisicion_datos_especializacion.md` §1.a documenta el éxito de esa adquisición solo para `municipal`/`provincial`). El titular real de octubre de 2001 a nivel nacional era la ALIANZA (Fernando de la Rúa) -- dato correcto por completitud histórica, pero sin ninguna fila del panel a la que aplicarlo. A nivel **provincial** y **municipal**, en cambio, la ventana sí existe y el titular de octubre de 2001 (antes de la crisis de diciembre) está identificado sin ambigüedad y es continuo en los dos casos: provincial = PARTIDO JUSTICIALISTA (Carlos Ruckauf, gobernador desde dic-1999; renuncia en dic-2001 para ser canciller, lo sucede su vicegobernador Felipe Solá -- sucesión constitucional, no ruptura, y Solá gana la reelección en 2003 con lista propia); municipal = PARTIDO JUSTICIALISTA (Julio Alak, intendente desde 1991, reelecto en 1995/1999/2003) -- ambos ya codificados como `continua` en `_TITULAR_INICIAL_2001`/`construir_oficialismo_por_nivel` (`src/ml_models/construir_calendario.py`) y verificados contra las filas reales de `oficialismo_por_nivel.csv` (`2001,provincial,PARTIDO JUSTICIALISTA,...,continua` / `2001,municipal,PARTIDO JUSTICIALISTA,...,continua`). **Resolución**: `sin_oficialismo` se retira de la lista de valores esperados en `especificacion_panel_temporal.md` §6.2, documentada ahí como categoría planificada en el diseño original que nunca llegó a activarse -- no un valor vigente del pipeline. Los tres valores reales que produce `construir_calendario.construir_oficialismo_por_nivel` son `continua`/`continua_renombrada`/`ruptura`, confirmado contra las 34 filas de `oficialismo_por_nivel.csv` (ninguna categoría más aparece). No requiere cambios de código: es una corrección de la especificación contra el comportamiento ya correcto del pipeline | Adoptada |
