...
There is no build/lint step configured. Tests only cover `src/electoral/models.py`
(pure parsing, no network) — `src/electoral/client.py` and `src/analisis/*`
have no automated tests; changes there are validated by re-running the
notebooks end to end (see README "Cómo reproducir").
...
- `ResultadosClient` wraps two distinct, differently-shaped endpoints of the
  ...
  **undocumented** endpoint discovered by reading the site's JS bundle; it
  ...
  - Disk cache path: `data/<anio_eleccion>/<categoria_nombre>/...`.
...
- **`data/<año>/<categoría|nivel>/<etapa>/`** where `etapa` is `generales`,
...
- **`src/analisis/`** reads only the derived `circuito_<nivel>.json` files
  ...
  under `graficos/`. Only `graficos/serie_temporal/` is git-tracked — the
  rest is `.gitignore`d and regenerated on demand from `data/`.
